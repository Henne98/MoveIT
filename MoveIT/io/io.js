...
module.exports = function (server) {
    ...
    // ------------------------------------------------------------------
    //  Alle Verbindungen in einen Room namens „lobby:{code}“ setzen
    // ------------------------------------------------------------------
    io.on('connection', socket => {
        console.log(`Client connected: ${socket.id}`);

        // Der Client meldet sich mit einem Lobby‑Code an
        socket.on('joinLobby', ({ lobbyCode }) => {
            const room = `lobby:${lobbyCode}`;
            socket.join(room);
            console.log(`Socket ${socket.id} joined ${room}`);
            
            // Optional: Liste der Spieler im Room zurücksenden
            io.to(room).emit('playerListUpdate', Array.from(io.sockets.adapter.rooms.get(room) || []));
        });

        // ------------------------------------------------------------------
        //  Host startet das Spiel – allen in diesem Room senden!
        // ------------------------------------------------------------------
        socket.on('startGame', ({ lobbyCode }) => {
            const room = `lobby:${lobbyCode}`;
            console.log(`Start game for ${room}`);
            
            // Sende an *alle* Clients im Room die Nachricht
