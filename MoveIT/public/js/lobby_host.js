...
const socket = io();          // Socket‑Verbindung zum Server herstellen
  document.getElementById('leave_lobby')
          .addEventListener('click', leaveLobby);

  function leaveLobby() {
      socket.emit('hostLeaving');           // Host‑Signal senden
      window.location.href = '/';           // Zurück zum Hauptmenü
  }